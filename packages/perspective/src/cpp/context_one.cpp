...
t_tscalvec
t_ctx1::get_data(t_tvidx start_row,
                 t_tvidx end_row,
                 t_tvidx start_col,
...
t_ctx1::get_pkeys(const t_uidxpvec& cells) const
{
    ...
    for (const auto& c : cells)
    {
        ...
        rval.insert(
            std::end(rval), std::begin(pkeys), std::end(pkeys));
    }
    return rval;
}
...
t_ctx1::get_column_dtype(t_uindex idx) const
{
    if (idx == 0 || idx >= static_cast<t_uindex>(get_column_count()))
        return DTYPE_NONE;
    return m_tree->get_aggtable()
        ->get_const_column(idx - 1)
        ->get_dtype();
}
...
t_ctx1::unity_get_column_names() const
{
    ...
    for (t_uindex idx = 0, loop_end = unity_get_column_count();
         idx < loop_end;
         ++idx)
    {
