...
module.exports = perspective => {
    describe("Parsed expressions", function() {
        ...
        it("Should be able to create a computed column in `view()` using a functional operator", async function() {
            ...
            const result = await view.to_columns();
            expect(result["custom column name"]).toEqual(common.int_float_data.map(x => Math.sqrt(x)));
            view.delete();
            table.delete();
        });
        ...
        it("Should be able to create a computed column in `view()` using a functional operator and 'AS'", async function() {
            ...
            const result = await view.to_columns();
            expect(result["custom column"]).toEqual(common.int_float_data.map(x => Math.sqrt(x)));
            view.delete();
            table.delete();
        });
